...
app.get("/paychangu/callback", (req, res) => {
  ...
  if (tx_ref) {
    ...
    const redirectUrl = `${frontendUrl}/sell?payment=success&tx_ref=${encodeURIComponent(tx_ref)}`;

    console.log(`➡️ Redirecting browser to frontend: ${redirectUrl}`);

    // Send an HTML page with JavaScript redirect + fallback meta refresh
    const html = `
      <!DOCTYPE html>
      <html>
        <head>
  ...
  console.log("⚠️ No tx_ref found – returning JSON fallback");
  return res.status(200).json({
    message: "Callback received (no tx_ref)",
    data: req.query
  });
});
// --------------------
...
app.post("/create-payment", async (req, res) => {
  try {
    ...
    // ✅ Return checkout_url in a structure the frontend expects
    res.json({
      status: 'success',
